...
namespace veritas{
    ...
    struct bound:bound_type<T,N>{
        ...
        bound(){for(int x=0;x<N;x++)mn[x]=fsytd::lim<T>::max(),mx[x]=fsytd::lim<T>::min();};
        ...
        void add(const vec<T,P,N>&p){mn=min(mn,p),mx=max(mx,p);}
        void add(const bound<T,N>&bd){mx=max(mx,bd.mx);mn=min(mn,bd.mn);}
        ...
        bool intersectP(const vec3<T,P>&o,const vec3<T,V>&d,T*near,T*far){
            if(fsytd::abs(d.x)<veritas::fsytd::lim<T>::eps()||fsytd::abs(d.y)<veritas::fsytd::lim<T>::eps()||fsytd::abs(d.z)<veritas::fsytd::lim<T>::eps())return 0;
