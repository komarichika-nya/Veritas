...
namespace veritas{
    ...
    template<typename T>struct quat{
        ...
        quat<T>unit(){return{T(1),T(0),T(0),T(0)};}
        quat<T>norm(const quat<T>&a){T v=sqrt(a.w*a.w+a.x*a.x+a.y*a.y+a.z*a.z);return{a.w/v,a.x/v,a.y/v,a.z/v};}
        quat<T>axis(const vec3<T,V>&a,const T ang){auto na=nor(a);return norm({cos(ang/T(2)),sin(ang/T(2))*na.x,sin(ang/T(2))*na.y,sin(ang/T(2))*na.z});}
        quat<T>vec(const vec3<T,V>&a,const vec3<T,V>&b){
        ...
        vec3<T,tp>rot(const quat<T>&q,const vec3<T,tp>&v){vec3<T,V>qv(q.x,q.y,q.z);return v+T(2)*q.w*cs(qv,v)+T(2)*cs(qv,cs(qv,v));}
        quat<T>matr(const mat34<T>&v){
        ...
        quat<T>conj()const{return{w,-x,-y,-z};}    
        ...
        quat<T>&operator*=(const quat&v){*this=(*this)*v;return *this;}
        quat<T>operator-(const quat<T>&a)const{return{w-a.w,x-a.x,y-a.y,z-a.z};}
        bool operator==(const quat<T>&a)const{return w-a.w<fsytd::lim<T>::eps()&&x-a.x<fsytd::lim<T>::eps()&&y-a.y<fsytd::lim<T>::eps()&&z-a.z<fsytd::lim<T>::eps();}
        quat<T>operator-()const{return{-w,-x,-y,-z};}
        bool same(const quat<T>&a,const quat<T>&b){return a==b||a==-b;}
        T qlen(const quat<T>&a){return sqrt(a.w*a.w+a.x*a.x+a.y*a.y+a.z*a.z);}
    };
    template<typename T>quat<T>slerp(quat<T>&a,quat<T>&b,T t){
