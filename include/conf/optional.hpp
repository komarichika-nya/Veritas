...
namespace fsytd{
    ...
    struct optional{
        ...
        HD optional(T&&val):has_val(true){new(buf)T(fsytd::forward_<T>(val));}
        HD optional(const optional&other):has_val(other.has_val){if(has_val)new(buf)T(other.val());}
        HD optional(optional&&other):has_val(other.has_val){if(has_val)new(buf)T(fsytd::forward_<T>(other.val()));other.reset();}
        ...
        HD optional&operator=(optional&&other)noexcept{
            ...
            if(has_val&&other.has_val)val()=fsytd::forward_<T>(other.val()),other.reset();
            else if(has_val)reset();
        ...
        template<typename...tp>HD T&emplace(tp&&...t){reset();new(buf)T(fsytd::forward_<tp>(t)...);has_val=1;return val();}
    };
}//namespace fsystd
