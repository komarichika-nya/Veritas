...
namespace veritas{
    ...
    public:
        ...
        Kd_tree(const Mesh<T>&m,const SplitMode&mode,T ct,T ci){
            ...
            idx.resize(tri->getMesh()->num_idx);
            std::iota(idx.begin(),idx.end(),0);
            init();
            switch(mode){
                case SplitMode::sah:root=this->build<SplitMode::sah>(0,static_cast<int>(idx.size())-1);break;
                case SplitMode::middle:root=this->build<SplitMode::middle>(0,static_cast<int>(idx.size())-1);break;
            }
        }
    private:
    ...
    private:
        void init(){
            ...
            lx=alloc_T.allocate(view->num_idx);
            rx=alloc_T.allocate(view->num_idx);
            ly=alloc_T.allocate(view->num_idx);
            ry=alloc_T.allocate(view->num_idx);
            lz=alloc_T.allocate(view->num_idx);
            rz=alloc_T.allocate(view->num_idx);
        }
        void update(int x){
        ...
        bool check(int x,ray<T>*r){
            T tmn=-fsytd::lim<T>::max(),tmx=fsytd::lim<T>::max();vec3<T,V>inv(T(1)/r->d.x,T(1)/r->d.y,T(1)/r->d.z);
            T tx1=(lx[x]-r->o.x)*inv.x;
            T tx2=(rx[x]-r->o.x)*inv.x;tmn=fsytd::max(tmn,fsytd::min(tx1,tx2));tmx=fsytd::min(tmx,fsytd::max(tx1,tx2));
            T ty1=(ly[x]-r->o.y)*inv.y;T ty2=(ry[x]-r->o.y)*inv.y;tmn=fsytd::max(tmn,fsytd::min(ty1,ty2));
            tmx=fsytd::min(tmx,fsytd::max(ty1,ty2));
            T tz1=(lz[x]-r->o.z)*inv.z;T tz2=(rz[x]-r->o.z)*inv.z;tmn=fsytd::max(tmn,fsytd::min(tz1,tz2));
    ...
    public:
        ~Kd_tree(){
            ...
            if(lx)alloc_T.deallocate(lx,view->num_idx);
            if(rx)alloc_T.deallocate(rx,view->num_idx);
            if(ly)alloc_T.deallocate(ly,view->num_idx);
            if(ry)alloc_T.deallocate(ry,view->num_idx);
            if(lz)alloc_T.deallocate(lz,view->num_idx);
            if(rz)alloc_T.deallocate(rz,view->num_idx);
