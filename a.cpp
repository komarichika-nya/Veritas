...
Spectrum<float>I(620,495,450);Transform<float>trans=Transform<float>::mv(vec3<float,P>(10,10,10));
...
Spectrum<float>render(ray<float>*r,int dep,Mesh<float>&mesh,Independent<float>&local,Kd_tree<float>&kd){
    ...
    if(tp==-1){//ground
        ...
        rho=Spectrum<float>::lerp(Spectrum<float>(1,1,1),Spectrum<float>(0.3,0.5,0.7),u);
    }  
    ...
    //check visable. spp=1.
    //LiSample
    Spectrum<float>d1,d2;//direct and indirect
    auto ctx=light.LiSample(sur,rng);Spectrum<float>li(0,0,0);float pdf;vec3<float,V>wi;
    if(ctx.has()){
        li=ctx.val().L;wi=ctx.val().wi;pdf=ctx.val().pdf;
        if(dot(wi,sur.n)>0&&dot(wi,ns)>0){
            ray<float>test;test.o=sur.pos+vec3<float,P>(sur.n.x,sur.n.y,sur.n.z)*eps,test.d=sur.t*wi;
            if(!fsytd::occluded(test,r->tmx,r->tmn,kd))d1=li*rho*(1.0f/veritas::fsytd::PI<float>)*dot(wi,sur.n)/pdf;
        }
    }
    vec3<float,V>dir=nor(sp(sur.n,local));ray<float>nxt;nxt.o=sur.pos+vec3<float,P>(sur.n.x,sur.n.y,sur.n.z)*eps,nxt.d=dir*sur.t;
