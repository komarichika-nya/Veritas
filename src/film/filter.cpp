...
HD vec2<T,P>Filter<tp,T>::radius(){return static_cast<const tp*>(this)->radius();}
template<typename tp,typename T>
...
template<typename T>
HD T Box<T>::evaluate(const vec2<T,P>&p)const{return (abs(p.x)<=r.x&&abs(p.y)<=r.y)?T(1):T(0);}
template<typename T>
